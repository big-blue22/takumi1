...
2. 各キャラクター行を処理（LUKE, JAMIE, MANON, KIMBERLY, MARISA等）
   - キャラクター名を英語で正確に抽出（全て大文字）
   - 「14勝/23戦」形式の場合:
     * "14"を勝利数(wins)として抽出
     * "23"を試合数(totalMatches)として抽出
     * 勝率を計算: (14/23)*100 = 60.87%
   - 勝率が直接表示されている場合はその値も確認
...
2. 各対戦相手行を処理（KIMBERLY, ZANGIEF, LILY, GUILE, JP等）
   - 対戦相手キャラクター名を英語で正確に抽出（全て大文字）
   - 「33戦」形式から試合数を抽出
   - 「54.55%」形式から勝率を抽出
   - 勝利数を計算: Math.round(試合数 * 勝率 / 100)
     例: 33戦 × 54.55% ≈ 18勝
...
すべてのエントリーに対して以下を実行：
1. **必須値の確保**
   - character: キャラクター名（大文字の英語名）
   - totalMatches: 試合数（整数）
   - winRate: 勝率（小数、0-100の範囲）
   - wins: 勝利数（整数）
...
⚠️ 重要な注意事項
═══════════════════════════════════════════════════════
1. 出力はJSON形式のみ。説明文や追加コメントは不要
2. キャラクター名は必ず大文字の英語表記（LUKE, JAMIE, KIMBERLY等）
3. 数値は正確に抽出・計算すること
4. 試合数0のキャラクターも必ず含める
5. 「ALL」は形式Aでのみ出現（総合成績として重要）
6. 画像が不鮮明でも、推測せず読み取れる範囲で正確に抽出
